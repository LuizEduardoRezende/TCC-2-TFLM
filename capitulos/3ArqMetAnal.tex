...
% Para a fase de testes e simulações, foi empregado o emulador QEMU\footnote{Disponível em: \url{https://www.qemu.org}} (versão 10.0.2), capaz de emular microcontroladores de famílias como STM32, que são baseados na arquitetura ARM Cortex-M e utilizados no projeto.
...
Para que a integração fosse desenvolvida corretamente, foi necessário projetar um programa exemplo inicial que contemplasse as novas palavras chaves, funcionalidades e fluxos de execução. Com base nesse programa, foi possível identificar quais modificações seriam necessárias na sintaxe da linguagem RL e quais funções do TFLM seriam necessárias.

% Essa tarefa também incluiu a determinação de quais funcionalidades seriam nativas da linguagem e quais pertenceriam à biblioteca padrão.
...
Para garantir uma avaliação justa, ambos os testes foram conduzidos em um MCU idêntico e com o mesmo modelo de ML. Nesta avaliação, foram considerados critérios como a facilidade de desenvolvimento, a manutenibilidade do código, a latência de inferência e o tamanho final do executável.
