...
No processo de validação da eficiência, os mesmos testes unitários foram utilizados para medir o desempenho dos três métodos de desenvolvimento de firmwares: utilizando a nova sintaxe da RL, utilizando a biblioteca padrão do Robcmp para TinyML e desenvolvendo diretamente em C++ com o TFLM. No processo de compilação foi uitlizada a flag \texttt{-Oz} para otimização do tamanho do binário final. Além disso, também foram utilziadas várias tags de otimização no processo de linkedição com o \textit{linker} Clang.
...
Para atingir resultador verídicos, os binários foram executados isoladamente, sem outros processos concorrentes na máquina de desenvolvimento. O tempo de medição utilizado foi a média do tempo real de execução (\textit{Time (mean)}) fornecido pelo Hyperfine.
...
\label{notebook-tamanho}
    ...
    Classificador de Spam & - & 591K & 188K \\ 
    Preditor de Seno quantizado & - & 512K & 88K \\ 
    Preditor de Seno & - & 512K & 88K \\ 
    \bottomrule
...
\label{comparativo}
    ...
    \toprule
    \textbf{Modelo TinyML} & \textbf{Sintaxe} & \textbf{Biblioteca padrão} & \textbf{TFLM(C++)}  \\ \midrule
    Classificador de Spam & - & 2.301 & 1.112 \\ 
    Preditor de Seno quantizado & - & 2.150 & 1.059 \\ 
    Preditor de Seno & - & 2.125 & 1.035 \\ 
    \bottomrule
