...
Neste capítulo são apresentados os detalhes da implementação das funcionalidades propostas neste trabalho, incluindo a construção da camada de interoperabilidade, a implementação da biblioteca padrão em RL e as adaptações realizadas no \textit{frontend} do compilador Robcmp.
...
É válido ressaltar que a biblioteca estática do TFLM pode ser facilmente compilada para diferentes arquiteturas, utilizando o sistema de \texttt{Makefile} do proprio \textit{framework}. Diferente do arquivo objeto do programa em RL, que é gerado pelo próprio compilador Robcmp. Por fim, a ponte entre a RL e o TFLM é feita pelo \textit{wrapper} C, que também é compilado por um sistema de Makefile dentro do diretório \textsl{test/tflm-tests}, gerando sua própria biblioteca estática.
...
\section{Implementação do \textit{wrapper} C}\label{sec:wrapper}
...
\begin{lstlisting}[style=rob, caption={Código proposto para a nova sintaxe que realiza a inferência de um modelo TFLite.}, label={lst:nova-sintaxe}]
int16 main() {
...
\begin{enumerate}
    \item Verifica se existe um valor à direita da atribuição: caso não exista o \textit{assignedValue}, um erro semântico é gerado. Afinal, apenas atribuições são permitidas para o tensor de entrada.
...
%\textbf{COMENTÁRIO LUIZ: Essa subsection foi implementada, porém não está funcionando corretamente.}
...
Um aspecto que pode ser melhorado é a convenção adotada para a entrada e saída de dados nos modelos. Atualmente, caso o desenvolvedor utilize variáveis ou vetores de ponto flutuante, a responsabilidade pela conversão e quantização recai sobre o \textit{wrapper}. Essa abordagem simplifica o uso da solução ao encapsular a complexidade, porém pode introduzir certas limitações. Por exemplo, ao utilizar métodos de arredondamento durante a quantização ou conversão de tipos, pode ocorrer perda de precisão, o que deve ser considerado em aplicações sensíveis a erros.
