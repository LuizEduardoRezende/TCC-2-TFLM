...
Neste capítulo são apresentados os detalhes da implementação das funcionalidades propostas neste trabalho, incluindo a contrução da camada de interoperabilidade, a implementação da biblioteca padrão em RL e as adaptações realizadas no \textit{frontend} do compilador Robcmp. Contudo, é importante ressaltar que, embora implementadas, essas adaptações no \textit{frontend} não foram totalmente integradas para a realização dos testes finais.
