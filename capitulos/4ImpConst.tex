...
É válido ressaltar que a biblioteca estática do TFLM pode ser facilmente compilada para diferentes arquiteturas, utilizando o sistema de \texttt{Makefile} do proprio \textit{framework}. Diferente do arquivo objeto do programa em RL, que é gerado pelo próprio compilador Robcmp. Por fim, o \textit{wrapper} C é compilado por meio do sistema de \texttt{CMakeLists} do Robcmp, que resolve todas as dependências e gera a biblioteca estática correspondente. A \hyperref[fig:linker]{Figura~\ref*{fig:linker}} ilustra a arquitetura de compilação e interoperabilidade entre os componentes.
...
\lstdefinestyle{cpp}{
    ...
    breaklines=true,
    captionpos=t,
    morekeywords={uintptr_t, int8_t, uint8_t, size_t}
}
...
\subsection{Processamento de Acesso a Membros (\texttt{.input}, \texttt{.output}, \texttt{.invoke})}
...
\subsection{\texttt{generateInputAccess} (\texttt{.input})}
...
\subsection{\texttt{generateOutputAccess} (\texttt{.output})}
...
\subsection{\texttt{generateInvoke} (\texttt{invoke})}
